...
import bf2
import math
from game.scoringCommon import addScore, RPL, hasPilotKit
...
def onGameStatusChanged(status):
    ...
    if status == bf2.GameStatus.Playing:
        ...
        for obj in g_controlPoints:
            ...
            obj.initialTeam = obj.cp_getParam('team')
            if obj.cp_getParam('team') > 0:
                obj.flagPosition = Top
            else:
                obj.flagPosition = Bottom
            
            # Disable forSoldierOnly
            game.utilities.rconExec('ObjectTemplate.activeSafe ControlPoint %s' % obj.templateName)
            game.utilities.rconExec('ObjectTemplate.forSoldierOnly 0')
        ...
        # Reduce HUD dummy flag spam
        for obj in g_controlPoints:
            if 'dummy' in obj.templateName.lower() and obj.initialTeam == 0:
...
def updateTicketLoss():
    ...
    for obj in g_controlPoints:
        if 'dummy' in obj.templateName.lower(): continue
...
def calcTicketLossForTeam(team, otherTeamAreaValue, otherTeamAreaOverweight):
    if otherTeamAreaValue >= 100 and otherTeamAreaOverweight > 0:
        return game.gameplayPlugin.getDefaultTicketLossPerMin(team) / 60.0
    else:
        return 0
...
def onCPTrigger(triggerId, cp, vehicle, enter, userData):
    ...
    if vehicle and enter:
        for p in vehicle.getOccupyingPlayers():
            ocp = getOccupyingCP(p)
            if ocp != None and ocp.isValid():
    ...
    for o in pcos:
        ...
        for p in occupyingPlayers:
            ...
            if p.isAlive() and hasPilotKit(p):
                continue
    ...
    attackOverWeight = 0
    needed = int(cp.getTemplateProperty('minNrToTakeControl'))
    
    if team1OverWeight > 0:
        attackingTeam = 1
        attackingTeamOccupants = team1Occupants
    elif team1OverWeight < 0:
        attackingTeam = 2
        attackingTeamOccupants = team2Occupants
    else:
        attackingTeam = 0
        attackingTeamOccupants = 0
    ...
    # don't enforce onlyTakeableByTeam on neutral flags which don't start neutral
    if cp.cp_getParam('onlyTakeableByTeam') != 0 and cp.cp_getParam('onlyTakeableByTeam') != attackingTeam \
       and (cp.cp_getParam('team') in [1,2] or cp.initialTeam == 0):
        return
    
    if team1Occupants > 0 or team2Occupants > 0:
        if attackingTeamOccupants < needed:
            return
        if g_plugin.veto_capture(cp, attackingTeam, attackingTeamOccupants):
            return
